...
function ThreeCardsSection({ cards }) {
  return (
    <section className="grid grid-cols-1 grid-row-1 sm:grid-cols-2 lg:grid-cols-3 max-w-screen-2xl sm:px-6 mx-auto snap-start">
      {cards &&
        cards.map((card) => <LinkedServiceCard key={card.id} {...card} />)}
    </section>
  )
}
